...
function slugifyContextBuilderPlaceholder(value: string, fallback: string): string {
...
function getContextBuilderBindings(node: GraphNode, graph: GraphDefinition): ContextBuilderBindingRow[] {
  ...
  const derivedBindings = incomingSources
    .filter((sourceNode) => !configuredSourceIds.has(sourceNode.id))
    .map((sourceNode, index) => ({
      sourceNodeId: sourceNode.id,
      sourceLabel: sourceNode.label,
      placeholder: slugifyContextBuilderPlaceholder(sourceNode.label, `source_${index + 1}`),
      binding: { type: "latest_payload", source: sourceNode.id },
      autoGenerated: true,
    }));

  return [...configuredBindings, ...derivedBindings];
}
